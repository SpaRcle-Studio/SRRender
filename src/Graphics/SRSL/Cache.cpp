...
namespace SR_SRSL_NS {
    namespace Details {
        void SaveExpression(SR_HTYPES_NS::Marshal& marshal, const SRSLExpr* pExpr);
        void SaveExpressions(SR_HTYPES_NS::Marshal& marshal, const std::vector<SRSLExpr*>& expressions);
        void SaveLexicalTree(SR_HTYPES_NS::Marshal& marshal, const SRSLLexicalTree* pTree);
        ...
        void SaveExpression(SR_HTYPES_NS::Marshal& marshal, const SRSLExpr* pExpr) {
            ...
            SR_UTILS_NS::MarshalUtils::SaveString(marshal, pExpr->token);
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, pExpr->isCall);
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, pExpr->isArray);
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, pExpr->isList);
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, pExpr->isString);

            SaveExpressions(marshal, pExpr->args);
        }

        void SaveExpressions(SR_HTYPES_NS::Marshal& marshal, const std::vector<SRSLExpr*>& expressions) {
            SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, expressions.size());
            for (const auto& arg : expressions) {
                SaveExpression(marshal, arg);
            }
        }

        void SaveDecorator(SR_HTYPES_NS::Marshal& marshal, const SRSLDecorator& decorator) {
            SR_UTILS_NS::MarshalUtils::SaveString(marshal, decorator.name);
            SaveExpressions(marshal, decorator.args);
        }

        void SaveDecorators(SR_HTYPES_NS::Marshal& marshal, const SRSLDecorators* pDecorators) {
            if (!pDecorators)  {
                SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, false);
                return;
            }
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, true);
            SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, pDecorators->decorators.size());
            for (const auto& decorator : pDecorators->decorators) {
                SaveDecorator(marshal, decorator);
            }
        }

        void SaveVariable(SR_HTYPES_NS::Marshal& marshal, const SRSLVariable* pVariable) {
            if (!pVariable) {
                SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, false);
                return;
            }
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, true);
            SaveDecorators(marshal, pVariable->pDecorators);
            SaveExpression(marshal, pVariable->pType);
            SaveExpression(marshal, pVariable->pName);
            SaveExpression(marshal, pVariable->pExpr);
        }

        void SaveVariables(SR_HTYPES_NS::Marshal& marshal, const std::vector<SRSLVariable*>& variables) {
            SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, variables.size());
            for (const auto& variable : variables) {
                SaveVariable(marshal, variable);
            }
        }

        void SaveUseStack(SR_HTYPES_NS::Marshal& marshal, const SRSLUseStack::Ptr& pUseStack) {
            if (!pUseStack) {
                SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, false);
                return;
            }
            ...
            for (const auto& [name, pFunction] : pUseStack->functions) {
                SR_UTILS_NS::MarshalUtils::SaveString(marshal, name);
                SaveUseStack(marshal, pFunction);
            }
        }

        void SaveLexicalUnit(SR_HTYPES_NS::Marshal& marshal, const SRSLLexicalUnit* pUnit) {
            if (!pUnit) {
                SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, false);
                return;
            }
            ...
            SR_UTILS_NS::MarshalUtils::SaveValue<uint32_t>(marshal, static_cast<uint32_t>(pUnit->GetLexicalUnitType()));

            switch (pUnit->GetLexicalUnitType()) {
                case LexicalUnitType::Expr:
                    SaveExpression(marshal, static_cast<const SRSLExpr*>(pUnit));
                    break;
                case LexicalUnitType::Decorator:
                    SaveDecorator(marshal, static_cast<const SRSLDecorator&>(*pUnit));
                    break;
                case LexicalUnitType::Decorators:
                    SaveDecorators(marshal, static_cast<const SRSLDecorators*>(pUnit));
                    break;
                case LexicalUnitType::Variable:
                    SaveVariable(marshal, static_cast<const SRSLVariable*>(pUnit));
                    break;
                case LexicalUnitType::Function: {
                    auto&& pFunction = static_cast<const SRSLFunction*>(pUnit);
                    SaveDecorators(marshal, pFunction->pDecorators);
                    SaveExpression(marshal, pFunction->pType);
                    SaveExpression(marshal, pFunction->pName);
                    SaveVariables(marshal, pFunction->args);
                    SaveLexicalTree(marshal, pFunction->pLexicalTree);
                    break;
                }
                case LexicalUnitType::Struct: {
                    auto&& pStruct = static_cast<const SRSLStructureStatement*>(pUnit);
                    SaveExpression(marshal, pStruct->pName);
                    SaveLexicalTree(marshal, pStruct->pLexicalTree);
                    break;
                }
                case LexicalUnitType::Return: {
                    auto&& pReturn = static_cast<const SRSLReturn*>(pUnit);
                    SaveExpression(marshal, pReturn->pExpr);
                    break;
                }
                case LexicalUnitType::IfStatement: {
                    auto&& pIf = static_cast<const SRSLIfStatement*>(pUnit);
                    SaveExpression(marshal, pIf->pExpr);
                    SaveLexicalTree(marshal, pIf->pLexicalTree);
                    SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, pIf->isElse);
                    break;
                }
                case LexicalUnitType::ForStatement: {
                    auto&& pFor = static_cast<const SRSLForStatement*>(pUnit);
                    SaveVariable(marshal, pFor->pVar);
                    SaveExpression(marshal, pFor->pCondition);
                    SaveExpression(marshal, pFor->pExpr);
                    SaveLexicalTree(marshal, pFor->pLexicalTree);
                    break;
                }
                case LexicalUnitType::WhileStatement: {
                    auto&& pWhile = static_cast<const SRSLWhileStatement*>(pUnit);
                    SaveExpression(marshal, pWhile->pCondition);
                    SaveLexicalTree(marshal, pWhile->pLexicalTree);
                    break;
                }
                case LexicalUnitType::LexcialTree:
                    SaveLexicalTree(marshal, static_cast<const SRSLLexicalTree*>(pUnit));
                    break;
                default:
                    SRHalt("Unsupported lexical unit type!");
                    break;
            }
        }

        void SaveLexicalUnits(SR_HTYPES_NS::Marshal& marshal, const std::vector<SRSLLexicalUnit*>& units) {
            SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, units.size());
            for (const auto& unit : units) {
                SaveLexicalUnit(marshal, unit);
            }
        }
        ...
        void SaveLexicalTree(SR_HTYPES_NS::Marshal& marshal, const SRSLLexicalTree* pTree) {
            ...
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, true);
            SaveLexicalUnits(marshal, pTree->lexicalTree);
        }

        void SaveAnalyzedTree(SR_HTYPES_NS::Marshal& marshal, const SRSLAnalyzedTree::Ptr& pTree) {
            SR_TRACY_ZONE;
            if (!pTree) {
                SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, false);
                return;
            ...
            SR_UTILS_NS::MarshalUtils::SaveValue<bool>(marshal, true);
            SaveLexicalTree(marshal, pTree->pLexicalTree);
        }
    }
    ...
    /*void SRSLShaderCache::SaveShaderToCache(const SR_UTILS_NS::Path& cachePath, const SR_SRSL_NS::SRSLShader* pShader) {
        ...
        marshal.Reserve(1024 * 128); // 128 KB

        SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, 0xDEADBEEF); // version
        ...
        Details::SaveCreateInfo(marshal, pShader->m_createInfo);
        Details::SaveSamplers(marshal, pShader->m_samplers);
        Details::SaveUniformBlock(marshal, pShader->m_pushConstants);
        Details::SaveUniforms(marshal, pShader->m_ssboBlocks);
        Details::SaveUniforms(marshal, pShader->m_uniformBlocks);

        SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, pShader->m_shared.size());
        for (const auto& [name, pVariable] : pShader->m_shared) {
            SR_UTILS_NS::MarshalUtils::SaveString(marshal, name);
            Details::SaveVariable(marshal, pVariable);
        }

        SR_UTILS_NS::MarshalUtils::SaveValue<uint64_t>(marshal, pShader->m_constants.size());
        for (const auto& [name, pVariable] : pShader->m_constants) {
            SR_UTILS_NS::MarshalUtils::SaveString(marshal, name);
            Details::SaveVariable(marshal, pVariable);
        }

        Details::SaveUseStack(marshal, pShader->m_useStack);
        Details::SaveAnalyzedTree(marshal, pShader->m_analyzedTree);

        if (cachePath.IsFile()) {
            SR_PLATFORM_NS::Delete(cachePath);
        }

        if (!marshal.Save(cachePath)) {
            SR_ERROR("SRSLShaderCache::SaveShaderToCache() : failed to save shader to cache! Path: {}", cachePath);
        }
    }
